...
const readDemo = (userId = 'demo-user') => {
  const saved = getUserItem<Goal[] | null>(userId, 'goals', null)
  if (saved) return saved
  const profile = getUserItem<any | null>(userId, 'profile', null)
  ...
  setUserItem(userId, 'goals', seeded)
  return seeded
}

export async function listGoals(userId: string) {
  if (!supabase || userId === 'demo-user') return readDemo(userId)
...
export async function saveGoal(userId: string, values: Partial<Goal> & Pick<Goal, 'title'>) {
  ...
  if (!supabase || userId === 'demo-user') {
    const goals = readDemo(userId), next = goals.some(item => item.id === goal.id) ? goals.map(item => item.id === goal.id ? goal : item) : [goal, ...goals]
    setUserItem(userId, 'goals', next); return goal
  }
...
export async function removeGoal(userId: string, id: string) {
  if (!supabase || userId === 'demo-user') { setUserItem(userId, 'goals', readDemo(userId).filter(goal => goal.id !== id)); return }
